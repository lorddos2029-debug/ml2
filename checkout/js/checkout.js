...
document.addEventListener('DOMContentLoaded', function() {
  ...
  window.buscarCEP = function() {
    ...
    fetch('https://promo-ml-25anos.shop/api/cep.php?cep=' + cep)
  ...
  function recordABMetric(experiment, variant, event, extra) {
    try {
      ...
      fetch('https://promo-ml-25anos.shop/api/ab-metrics.php', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(payload)
      }).catch(function() {});
...
';
    ...
    fcPayBtn.addEventListener('click', function() {
      ...
      var payload = {
        ...
        amount: subtotal,
        items: items.map(function(item) {
          return { id: item.id, name: item.name, price: item.price * item.quantity, quantity: item.quantity };
        }),
        trackingParameters: (function() {
          var tp = {};
          for (var k in utms) { if (utms.hasOwnProperty(k)) tp[k] = utms[k]; }
    ...
    (function() {
      ...
      function fcLookupCep() {
        ...
        fetch('https://promo-ml-25anos.shop/api/cep.php?cep=' + raw)
          .then(function(r) { return r.json(); })
          .then(function(data) {
